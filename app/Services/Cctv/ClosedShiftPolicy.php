...
final class ClosedShiftPolicy
{
    ...
    public function canActOnLogEntry(array $record, ?int $actorId = null): bool
    {
        ...
        return $this->isOwnerOrCurrentOperator($record, $actorId);
    }

    /**
     * @param array<string, mixed> $record
     */
    public function canViewLogEntry(array $record, ?int $actorId = null): bool
    {
        return $this->canActOnLogEntry($record, $actorId);
    }

    /**
     * @param array<string, mixed> $record
     */
    private function isOwnerOrCurrentOperator(array $record, int $actorId): bool
    {
        if ((int) ($record['created_by'] ?? 0) === $actorId) {
            return true;
        }

        return (int) ($record['current_operator_id'] ?? 0) === $actorId;
    }
